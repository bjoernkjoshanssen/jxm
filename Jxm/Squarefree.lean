...
instance (b:ℕ) (w : List (Fin b)) : Decidable (squarefree w) := by
  unfold squarefree
  exact w.length.decidableBallLT fun n _ => ∀ (v : Mathlib.Vector (Fin b) n), v.1 ≠ [] → ¬v.1 ++ v.1 <:+: w


instance (b:ℕ) (w : List (Fin b)) : Decidable (cubefree w) := by
  unfold cubefree
  exact
    w.length.decidableBallLT fun n _ => ∀ (v : Mathlib.Vector (Fin b) n), v.1 ≠ [] → ¬v.1 ++ v.1 ++ v.1 <:+: w
