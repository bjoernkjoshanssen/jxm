...
import Ptyche.HydrophobicPolarModelBasic
...
def morphSepᵥ {l:ℕ}
    ...
    (go₀ : Fin b₀ → α → α) (v : List.Vector (Fin b₀) l) : List.Vector (Fin b₁) l :=
...
def morphᵥ {l:ℕ} {α:Type} [Zero α] {b₀ b₁ : ℕ} (f : Fin b₀ → α → Fin b₁) (go₀ : Fin b₀ → α → α)
    (v : List.Vector (Fin b₀) l) : List.Vector (Fin b₁) l :=
  ⟨morph f go₀ v.1, by rw [morph_len];exact v.2⟩

/-- . -/
def morf {l:ℕ} {b₀ b₁ : ℕ} (f : Fin b₀ → Fin b₁) (v : List.Vector (Fin b₀) l) :
    List.Vector (Fin b₁) l := List.Vector.map f v
...
abbrev σ {l:ℕ} {α:Type} [Zero α] {b₀ b₁ : ℕ} (f : Fin b₀ → α → Fin b₁) (go₀ : Fin b₀ → α → α)
    (v : List.Vector (Fin b₀) l)  := morphᵥ f go₀ v
...
theorem pt_loc_of_embeds {α:Type} [DecidableEq α] {b₀ b₁ : ℕ}
    ...
    (fold : List.Vector α l) (a b : Fin l) (phobic : List.Vector Bool l)
...
theorem pts_at_of_embeds' {α:Type} [DecidableEq α] {b₀ b₁ : ℕ}
    {go₀ : Fin b₀ → α → α} {go₁ : Fin b₁ → α → α} (h_embed : embeds_in go₀ go₁)
    {l:ℕ} (k:Fin l) (ph : List.Vector Bool l) (fold : List.Vector α l):
    pts_at' go₀ k ph fold ≤
...
def pts_tot' {α:Type} {β : Type} [Fintype β] (go : β → α → α) [DecidableEq α] {l:ℕ}
    (ph : List.Vector Bool l)(fold : List.Vector α l) := ∑ i : Fin l, pts_at' go i ph fold
...
def points_tot {α β : Type} [DecidableEq α] [Fintype β] (go : β → α → α)
    {l:ℕ} (ph : List.Vector Bool l) (fold : List.Vector α l) :=
...
theorem pts_tot'_eq_points_tot {α:Type} {β : Type} [Fintype β] (go : β → α → α)
    [DecidableEq α] {l:ℕ} (ph : List.Vector Bool l) (fold : List.Vector α l) :
...
def pts_at_improved {α:Type} {β : Type} [Fintype β] (go : β → α → α)
    [DecidableEq α] {l:ℕ} (k : Fin l) (ph : List.Vector Bool l) (fold : List.Vector α l) : ℕ :=
  Finset.card (
...
lemma pts_at_bound'_improved {α:Type} [DecidableEq α]
    {β : Type} [Fintype β]
    (go : β → α → α)
    {l:ℕ} (ph : List.Vector Bool l) (fold : List.Vector α l) (i:Fin l):
...
theorem pts_earned_bound_loc'_improved
    ...
    {l:ℕ} (ph : List.Vector Bool l.succ) (fold : List.Vector α l.succ):
  ...
  unfold pts_tot'; apply Finset.sum_le_sum; intro i _
...
lemma path_len {α: Type} [Zero α] {β: Type} (go: β → α → α) {l: ℕ}
    (moves: List.Vector β l) : (path go moves.1).1.length = l.succ := by rw [(path go moves.1).2]; simp

/-- . -/
lemma path_at_len {α: Type} (base :α) {β: Type}
    (go: β → α → α) {l: ℕ} (moves: List.Vector β l) :
    (path_at base go moves.1).1.length = l.succ := by rw [(path_at base go moves.1).2]; simp

/-- . -/
def pathᵥ {l:ℕ}{α:Type} [Zero α] {β : Type} (go : β → α → α)
    (moves : List.Vector β l) : List.Vector α l.succ := ⟨(path go moves.1).1,path_len _ _⟩

/-- . -/
abbrev π  {l:ℕ}{α:Type} [Zero α] {β : Type}  (go : β → α → α)
    (moves : List.Vector β l) := pathᵥ go moves

/-- . -/
lemma pathᵥ_len {α: Type} [Zero α] {β: Type}
    (go: β → α → α) {l: ℕ} (moves: List.Vector β l) : (pathᵥ go moves).length = l.succ := by simp

/-- . -/
def pathᵥ_at {l:ℕ}{α:Type} (base : α) {β : Type} (go : β → α → α)
    (moves : List.Vector β l) : List.Vector α l.succ :=
  ⟨(path_at base go moves.1).1,path_at_len _ _ _⟩

/-- . -/
def pt_dir {α:Type} [Zero α] {β : Type} (go : β → α → α)
    {l:ℕ} (j : Fin l.succ) (moves: List.Vector β l) (ph : List.Vector Bool l.succ) :
...
theorem unique_loc  {α: Type} [Zero α] {β: Type}
    {go: β → α → α}
    {l:ℕ} {j: Fin l.succ}
    {moves: List.Vector β l} {ph : List.Vector Bool l.succ}
...
theorem unique_loc_dir_rect {l:ℕ} (j: Fin l.succ)
    (moves: List.Vector (Fin 4) l) (ph : List.Vector Bool l.succ)
...
theorem unique_loc_dir_hex {l:ℕ} (j: Fin l.succ)
    (moves: List.Vector (Fin 6) l) (ph : List.Vector Bool l.succ)
...
instance  {α: Type} [Zero α] [DecidableEq α] {b:ℕ}
    {go: Fin b → α → α}
      {l:ℕ} (j : Fin l.succ) (ph : List.Vector Bool l.succ)
        (moves: List.Vector (Fin b) l) (a : Fin b)
    (i : Fin l.succ) : Decidable (pt_dir go j moves ph a i) :=
  decidable_of_iff (List.Vector.get ph i = true ∧
    List.Vector.get ph j = true ∧
      Nat.succ ↑i < ↑j ∧
      List.Vector.get (pathᵥ go moves) j = go a (List.Vector.get (pathᵥ go moves) i)) (by
        unfold pt_dir;tauto
      )
...
theorem pts_at'_dir {α: Type} [Zero α] [DecidableEq α] {b:ℕ} {go: Fin b → α → α}
    {l:ℕ} (j : Fin l.succ) (ph : List.Vector Bool l.succ)
    (moves: List.Vector (Fin b) l)
    (path_inj : (Function.Injective fun k => List.Vector.get (pathᵥ go moves) k))
    (right_inj: right_injective go)
...
theorem pts_earned_bound_dir' {α: Type} [Zero α] [DecidableEq α] {b:ℕ}
    {go: Fin b → α → α}
    {l:ℕ} (ph : List.Vector Bool l.succ)
    (moves: List.Vector (Fin b) l)
    (path_inj  : (Function.Injective fun k => List.Vector.get (pathᵥ go moves) k))
    (right_inj : right_injective go)
...
/-- . -/
theorem two_heads {α : Type} {k :ℕ} (v: List.Vector α k.succ) (w : List α) (hw : w ≠ [])
    (h : v.1 = w) : List.Vector.head v = List.head w hw := by
  symm at h
  cases w with
  |nil => tauto
  |cons head tail =>
    ...
    have : v = head ::ᵥ ⟨tail, this⟩ := by
      ...
      simp only [List.Vector.toList_cons, List.Vector.toList_mk]
      rw [h]
      rfl
    rw [this]; simp
...
/-- . -/
lemma ne_nil_of_succ_length {α :Type} {k:ℕ} (tail_ih: List.Vector α k.succ) : tail_ih.1 ≠ [] := by
...
lemma path_eq_path_morph {α:Type} [Zero α] {b₀ b₁ : ℕ} (f : Fin b₀ → α → Fin b₁)
  ...
  (path go₀ moves).1 = (path go₁ (morph f go₀ moves)).1 := by
    ...
    |cons head tail tail_ih =>
      rw [path_cons,g head (List.Vector.head (path go₀ tail)),tail_ih ]
      unfold morph; simp only [List.length_cons];
      rw [path_cons]; simp only [List.cons.injEq, and_true]
      have : (List.Vector.head (path go₀ tail))
        = (List.Vector.head (path go₁ (List.rec []
        (fun head tail tail_ih => f head (List.Vector.head (path go₀ tail)) :: tail_ih) tail))) := by
...
lemma path_eq_path_morphᵥ {l:ℕ} {α:Type} [Zero α] {b₀ b₁ : ℕ}
    ...
    (g : is_embedding go₀ go₁ f) (moves : List.Vector (Fin b₀) l) :
...
lemma pathᵥ_eq_path_morphᵥ {l:ℕ} {α:Type} [Zero α] {b₀ b₁ : ℕ}
  ...
  List.Vector.eq (pathᵥ go₀ moves) (pathᵥ go₁ (morphᵥ f go₀ moves))
    <| pathᵥ_eq_path_morphᵥ1 f go₀ go₁ g moves

/-- . -/
def path_transformed {α: Type} [Zero α] {b₀ b₁: ℕ}
    (f: Fin b₀ → α → Fin b₁) (go₀: Fin b₀ → α → α) (go₁: Fin b₁ → α → α)
    (l: List (Fin b₀)) : List.Vector α l.length.succ :=
  ⟨
...
def path_transformedᵥ {l:ℕ} {α: Type} [Zero α] {b₀ b₁: ℕ}
    (f: Fin b₀ → α → Fin b₁) (go₀: Fin b₀ → α → α) (go₁: Fin b₁ → α → α)
    (v: List.Vector (Fin b₀) l) : List.Vector α l.succ :=
...
theorem transform_of_embed {α:Type} [Zero α] {b₀ b₁ : ℕ}
  ...
  |cons head tail tail_ih =>
    ...
    constructor
    · let a := (List.Vector.head (path go₀ tail))
      rw [h_embed head a]
      have : path go₁ (morph f go₀ tail)
        = ⟨(path go₀ tail).1,(by rw [morph_len]; exact (path go₀ tail).2)⟩ :=
        List.Vector.eq _ _ (by unfold List.Vector.toList; rw [← tail_ih])
      rw [this]
      have hau: ∃ a u, path go₀ tail = a ::ᵥ u := List.Vector.exists_eq_cons (path go₀ tail)
      have : List.Vector.head ⟨
        (path go₀ tail).1, by
          show (path go₀ tail).1.length = (morph f go₀ tail).length.succ
          rw [morph_len]; exact (path go₀ tail).2
        ⟩ = List.Vector.head (path go₀ tail) := by
        obtain ⟨a,ha⟩ := hau
        obtain ⟨u,hu⟩ := ha
        rw [hu]; simp only [List.Vector.cons_val, List.Vector.head_cons]; rfl
...
def pts_tot_bound {α:Type} [Zero α] [DecidableEq α]
    {β : Type} [Fintype β]
    (go : β → α → α) {l:ℕ} (ph : List.Vector Bool l.succ) (q : ℕ) :=
    ∀ moves : List.Vector β l, Function.Injective (fun k ↦ (path go moves.1).1.get k) →
    pts_tot' go ph (⟨(path go moves.1).1,path_len _ _⟩) ≤ q

-- def myList {b l:ℕ} := Finset.toList (Finset.univ : Finset (List.Vector (Fin b) l))
...
/-- . -/
instance {l:ℕ} {α:Type} [Zero α] [DecidableEq α] {β : Type} [Fintype β]
    (go : β → α → α) {ph : List.Vector Bool l.succ} :
...
theorem pts_tot_bound_rev_exists {α:Type} [Zero α] [DecidableEq α] {β : Type} [Fintype β]
    (go : β → α → α) {l:ℕ} (ph : List.Vector Bool l.succ) :
...
def HP {α:Type} [Zero α] [DecidableEq α] {β : Type} [Fintype β]
    (go : β → α → α) {l:ℕ} (ph : List.Vector Bool l.succ) :ℕ := Nat.find (pts_tot_bound_exists go ph)
...
def HP_rev {α:Type} [Zero α] [DecidableEq α] {β : Type} [Fintype β]
    (go : β → α → α) {l:ℕ} (ph : List.Vector Bool l.succ) : ℕ :=
  Nat.find (pts_tot_bound_rev_exists go ph)

/-- . -/
def P_tri'  {l:ℕ} := fun ph : List.Vector Bool l.succ ↦ HP tri  ph
/-- . -/
def P_rect' {l:ℕ} := fun ph : List.Vector Bool l.succ ↦ HP rect ph
/-- . -/
def P_rect₃' {l:ℕ} := fun ph : List.Vector Bool l.succ ↦ HP_rev rect₃ ph
/-- . -/
def P_hex'  {l:ℕ} := fun ph : List.Vector Bool l.succ ↦ HP hex  ph
-- Is P_rect₃' ≥ P_tri' ?
...
/-- . -/
theorem List.Vector_inj_of_list_inj {t : Type} {n : ℕ}
    {v : List.Vector t n} (h: Function.Injective fun k => List.get v.1 k) :
    Function.Injective fun k => List.Vector.get v k := by
...
/-- . -/
theorem list_inj_of_List.Vector_inj {t : Type} {n : ℕ}
    {v : List.Vector t n} (h: Function.Injective fun k => List.Vector.get v k) :
  ...
  have hy: y.1 < n := by
    let Q := y.2;have : v.1.length = n := v.2;simp_rw [this] at Q;exact Q
  have : List.Vector.get v ⟨x.1,hx⟩ = List.Vector.get v ⟨y.1,hy⟩ := hxy
  let Q := h this; simp only [Fin.mk.injEq] at Q hxy
  exact Fin.ext Q

/-- . -/
theorem P_tri_lin_bound {l:ℕ} (ph : List.Vector Bool l.succ) :
    P_tri' ph ≤ l.succ * 3 :=
  Nat.find_le (fun _ path_inj =>
    pts_earned_bound_dir' _ _ (List.Vector_inj_of_list_inj path_inj)
      right_injective_tri left_injective_tri)

/-- . -/
theorem P_hex_lin_bound {l:ℕ} (ph : List.Vector Bool l.succ) :
    P_hex' ph ≤ l.succ * 6 :=
  Nat.find_le (fun _ path_inj =>
    pts_earned_bound_dir' _ _ (List.Vector_inj_of_list_inj path_inj)
...
theorem value_bound_of_embeds_strongly {α:Type} [Zero α] [DecidableEq α] {b₀ b₁ : ℕ}
    ...
    {l:ℕ} (ph : List.Vector Bool l.succ) : HP go₀ ph ≤ HP go₁ ph :=
  ...
  let moves' :=
    (⟨morph f go₀ moves.1, (by rw [morph_len]; exact moves.2)⟩ : List.Vector (Fin b₁) l)
  calc
...
-- abbrev t := true
-- abbrev f := false
-- #eval numtrue   (⟨[f,f,f,f,f,t],rfl⟩ : List.Vector Bool 6)

-- abbrev xx := (⟨[f,f,f,f,f,f],rfl⟩ : List.Vector Bool 6)
-- abbrev y := [
